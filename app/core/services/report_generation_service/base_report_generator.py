...
from app.common.exceptions import QueryExecutionError
...
class BaseGenerator(ABC):
    ...
    def generate(
        ...
        try:
            ...
            self._logger.debug(f"Generation step {step}: Creating predefined tables")
            self._create_predefined_tables()
            self._logger.info("Predefined tables created successfully")
    ...
    def _generate_output_filename(self) -> str:
        from app.core.services.date_formatting_service import DateFormattingService
        ...
        output_filename = output_filename.replace(
            "{date}",
            DateFormattingService.to_french_filename_date(
                self._report_context.reporting_date
            ),
        )
